...
print(df.shape) #tuple of rows and columns
print(df.info())
...
print(clean_data(df)) #function to check and fill missing data and drop duplicates

# Comment on data types and suggest optimizations for memory efficiency.
df['Temperature(°C)'] = pd.to_numeric(df['Temperature(°C)'], downcast='float') #smallest possible floating-point type (float32 instead of the default float64)
df['Humidity(%)'] = pd.to_numeric(df['Humidity(%)'], downcast='integer') #downcast to the smallest integer type (int8, int16)
df['Wind speed (m/s)'] = pd.to_numeric(df['Wind speed (m/s)'], downcast='float') #smallest possible floating-point type (float32 instead of the default float64)
